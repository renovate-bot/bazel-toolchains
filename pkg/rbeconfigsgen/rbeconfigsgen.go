...
`))
	...
	// licenseBlob contains the Apache 2 license that will be included with generated configs.
	// Can't embed the LICENSE file at the root of this repo because the embed directive doesn't
	// accept ".." in the path to the embedded file. So we have a local copy of the license instead.
	//go:embed apache-2_configs_license.txt
	licenseBlob []byte
)
...
func genCppConfigs(d *dockerRunner, o *Options, bazelPath string) (string, error) {
	...
	cppConfigDir := path.Join(bazelOutputRoot, "external", cppConfigRepo)
...
// cppConfigTargetAndRepo returns the C++ autoconfig build target and the external repository name
// where the generated configs will be located, based on the Bazel version.
func cppConfigTargetAndRepo(bazelVersion string) (string, string) {
	useBzlmod := !isBazelVersionLessThan(bazelVersion, "8.0.0")

	cppConfigTarget := "@local_config_cc//..."
	cppConfigRepo := "local_config_cc"

	if useBzlmod {
		cppConfigTarget = "@@rules_cc++cc_configure_extension+local_config_cc//..."
		cppConfigRepo = "rules_cc++cc_configure_extension+local_config_cc"
	}
	return cppConfigTarget, cppConfigRepo
}
